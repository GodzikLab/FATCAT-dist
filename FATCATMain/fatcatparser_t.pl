...
my $usage = 
    ...
    "  Possible function:\n".
    "        -f report      (report the alignment parameters, default)\n".
    "        -f reportall   (report the alignment parameters and the superposed alignment)\n".
    "        -f reportali   (report the superposed alignment only)\n".
...
if ($func =~ /^report$/i) {
    printReport($ali_f, $gap, $twist, $alilen, $equlen, $rmsd, $pval, $out_f, "report");
} elsif ($func =~ /^reportali$/i) {
    printReport($ali_f, $gap, $twist, $alilen, $equlen, $rmsd, $pval, $out_f, "ali");
} elsif ($func =~ /^reportall$/i) {
    printReport($ali_f, $gap, $twist, $alilen, $equlen, $rmsd, $pval, $out_f, "all");
...
sub printReport {
    my ($ali_f, $gap, $twist, $alilen, $equlen, $rmsd, $pval, $out_f, $what) = @_;
	...
	if ($what eq "report") {
	    ...
	    print OUT "# pdb1 pdb1 start-pos-1 start-pos-2 ali\n";
	} elsif ($what eq "all") {
	    print OUT "# pdb1 pdb1 len1 len2 twist iniLen iniRmsd optLen optRmsd chainRms score alnLen gap pval AfpNum Identity(%) Similarity(%) start-pos-1 start-pos-2 ali\n";
	}
    }
	...
	if ($l =~ /^Align/) {
	    @columns = parseOnePair($aln);
	    my $report_line = printOnePair(\@columns, $gap, $twist, $alilen, $equlen, $rmsd, $pval, $what);
...
sub parseOnePair{
    ...
    my @lines = split(/\n/, $aln);
    my $start1;  # start position of protein 1 in alignment
    my $start2;  # start position of protein 2 in alignment
    my $i = 0;
    ...
    return ($protein_1, $len1, $protein_2, $len2, $factor, "$chain_1:$blockIndex:$chain_2", $start1, $start2);
}
	    
	

sub  printOnePair {
    my ($columns_p, $gap, $twist, $alilen, $equlen, $rmsd, $pval, $what) = @_;

    my ($protein_1, $len1, $protein_2, $len2, $factor, $ali, $start1, $start2) = @$columns_p;
    if (!$factor || !$ali) { return ""; } # no alignment
	...
	return "$protein_1\t$protein_2\t$start1\t$start2\t$ali\n";
    } elsif ($what eq "all") {
	return "$protein_1\t$len1\t$protein_2\t$len2\t$factor\t$start1\t$start2\t$ali\n";
    }
}
